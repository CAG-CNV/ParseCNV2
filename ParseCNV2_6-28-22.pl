...
use Getopt::Long;
GetOptions('i=s'=>\$input,'o=s'=>\$out,'b=s'=>\$build,'p=f'=>\$mergePVar,'d=i'=>\$mergeDist,'t'=>\$tdt,'m=f'=>\$maxPInclusion,'bfile=s'=>\$bfile,'qc'=>\$qc,'log=s'=>\$log,'c=s'=>\$cases,'q=s'=>\$quantitativeTrait,'batch=s'=>\$batch,'covar=s'=>\$covar);
...
if($input =~ /\.txt$/)
{
	...
	{       ##TODO get build from vcf
		##hg19 GRCh37 hg38 GRCh38 NCBI36 hg18 NCBI35 hg17 NCBI34 hg16
...
else
{
	...
	if($quantitativeTrait ne "")
	{
		...
		$c="awk '{ORS=\"\";if(NR==1)print NF-1}' $quantitativeTrait";$traits=`$c`;
...
#--glm Basic association analysis on quantitative and/or case/control phenotypes. For each variant, a linear (for quantitative traits) or logistic (for case/control) regression is run with the phenotype as the dependent variable, and nonmajor allele dosage(s) and a constant-1 column as predictors.
		...
		if($quantitativeTrait ne "")
		{
			$c=$MyDirectoryPathPrefix."PerlModules/./plink --bfile temp/$out"."del --assoc --pheno $quantitativeTrait --allow-no-sex --out temp/$out"."del";if($covar ne ""){$c=$c." --covar $covar";};$o=`$c`;
			$c=$MyDirectoryPathPrefix."PerlModules/./plink --bfile temp/$out"."dup --assoc --pheno $quantitativeTrait --allow-no-sex --out temp/$out"."dup";if($covar ne ""){$c=$c." --covar $covar";};$o=`$c`;
			$c=$MyDirectoryPathPrefix."PerlModules/./plink --bfile temp/$out"."deldup --assoc --pheno $quantitativeTrait --allow-no-sex --out temp/$out"."deldup";if($covar ne ""){$c=$c." --covar $covar";};$o=`$c`;
		}
		if($tdt ne "")
        	{
...
$mySnpColNum=$i;
print "myPColNum mySNPColNum myBetaTColNum myORColNum myChrColNum myBpColNum mySnpColNum\n";
print $myPColNum."\t".$mySNPColNum."\t".$myBetaTColNum."\t".$myORColNum."\t".$myChrColNum."\t".$myBpColNum."\t".$mySnpColNum."\n";
...
while($snpStat=<DEL>)
{
	...
	@stat=split(/\s+/,$snpStat);
        ...
        #}
	if($stat[$myPColNum]<=$maxPInclusion && $stat[$myPColNum] ne "NA")
	{
